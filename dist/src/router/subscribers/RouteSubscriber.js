...
class RouteSubscriber extends SubscriberBase_1.SubscriberBase {
    process(msg) {
        ...
        (0, RouterUtils_1.handle)(data.context, data.body ? ProtocolMgr_1.protoMgr.decodeMsgBody(data.body, data.context.protoId) : undefined, this.server)
            .then((result) => {
            if (msg.reply) {
                msg.respond((0, RouterUtils_1.encodeRouterPack)({ id: data.context.id }, result ? ProtocolMgr_1.protoMgr.encodeMsgBody(result, data.context.protoId) : undefined));
            }
        })
            .catch((e) => {
            (0, Logger_1.logErr)(e);
